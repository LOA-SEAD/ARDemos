   ...
   // addListeners();
    ...
    $('#save').click(function () {
        ...
        if(contador == 1) {
            ...
            window.top.location.href = "/scalear/model3D/finish/" + params;
        }
        else{
            $('#totalResource').empty();
            $("#totalResource").append("<div> <p> Selecione apenas um modelo antes de enviar. </p> </div>");
            $('#infoModal').openModal();
        }
...
function save(tr) {
    ...
    var url = location.origin + '/scalear/model3D/save/';
...
function update(tr) {
    ...
    var url = location.origin + '/scalear/model3D/update/' + $(tr).attr('data-id');
    var data = { name: $(tds)[1].textContent,
                 source: $(tds)[2].textContent,
                 category: $(tds)[3].textContent,
                 author: $(tds)[4].textContent,
...
function _edit(tr){
    var url = location.origin + '/scalear/model3D/returnInstance/' + $(tr).attr('data-id');
    ...
    $.ajax({
            ...
            success: function (returndata) {
                ...
                // Name - Source - author - Version - ownerId - taskId - ID
                ...
                $("#editVersion").attr("value",resourceInstance[3]);
                $("#modelID").attr("value",resourceInstance[6]);
    ...
    if(list_id.length<=0){
        ...
        if(list_id.length==1){
            if(confirm("Você tem certeza que deseja deletar essa questão?")){
                url = location.origin + '/scalear/model3D/delete/' + list_id[0];
                data = {_method: 'DELETE'};
                trID = "#tr"+list_id[0];
                $.ajax({
                        type: 'DELETE',
            ...
            if(confirm("Você tem certeza que deseja deletar esses modelos?")){
                for(var i=0;i<list_id.length;i++){
                    url = location.origin + '/scalear/model3D/delete/' + list_id[i];
                    data = {_method: 'DELETE'};
                    trID = "#tr"+list_id[i];
                    $(trID).remove();
                    $.ajax({
