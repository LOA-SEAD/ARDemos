...
class QuestionController {
    ...
    def exportQuestions(){
        ...
        for (int i = 0; i < list_questionId.size();i++){
            ...
            createTxtFile(fileName, Question.findById(list_questionId[i]))
            def id = MongoHelper.putFile(folder + '/' + fileName)
            files += "files=${id}"
            if (i != 3) files += "&"
        }
    ...
    void createTxtFile(String fileName, Question question){
