...
function submit(){
    ...
    if (list_id.length == 4) {
        //Chama controlador para salvar questões nos arquivos

        $.ajax({
            type: "POST",
            traditional: true,
            url: "/ardemo/question/exportQuestions",
        ...
        $('#totalQuestion').empty();
        $('#totalQuestion').append("<div> <p> Você deve selecionar 4 questões para serem utilizadas.</p> </div>");
        $('#infoModal').openModal();
    }
}

function _edit(tr){
    var url = location.origin + '/ardemo/question/returnInstance/' + $(tr).attr('data-id');
    ...
    if(list_id.length<=0){
        ...
        if(list_id.length==1){
            if(confirm("Você tem certeza que deseja deletar essa questão?")){
                url = location.origin + '/ardemo/question/delete/' + list_id[0];
                data = {_method: 'DELETE'};
                trID = "#tr"+list_id[0];
                $.ajax({
                        type: 'DELETE',
            ...
            for(var i=0;i<list_id.length;i++){
                    url = location.origin + '/ardemo/question/delete/' + list_id[i];
                    data = {_method: 'DELETE'};
                    trID = "#tr"+list_id[i];
                    $(trID).remove();
                $.ajax({
