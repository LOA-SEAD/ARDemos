...
public class I_QuestionController1 : MonoBehaviour
{
    ...
    [SerializeField] public bool useNextQuestionButton = false;
    [SerializeField] private Button nextQuestionButton;
    [SerializeField] public GameObject nextQuestion;
    [SerializeField] public bool usePreviousQuestionButton = false;
    [SerializeField] private Button previousQuestionButton;
    [SerializeField] public GameObject previousQuestion;
    ...
    private I_QuestionController1[] questionController1Components;
    private QuestionController2[] questionController2Components;

    //controle da importacao do modelo
    public MultiObjectImporter multiobjectimporter;
    private bool timer_start_when_load = true;
    private bool loaded = false;
    private Text[] questions;

    //existe apra ajudar a integracao com a activateInformation
    public bool Controler_aux;
    ...
    private void Awake()
    {
        questions = new Text[Texts.Count];
        Controler_aux = true;
        for (int aux = 0; aux < Texts.Count; aux++)
        {
            questions[aux] = Texts[aux];
        }
        //MENSAGEM DE ERRO SEVERAL QUESTIONCONTROLLER ACTIVE
        questionController1Components = GameObject.FindObjectsOfType<I_QuestionController1>();
        ...
        if (Active)
        {
            ...
            foreach (I_QuestionController1 questionController1 in questionController1Components)
            {
    ...
    void Start()
    {
        ...
        if (useNextQuestionButton)
        {
            ...
            Debug.Log("Indo para a proxima questao");
    ...
    void Update()
    {
        //entra uma unica vez quando ativo para carregar o modelo e os textos
        if (active)
        {
            //CODIGO RELACIONADO COM LEITURA DOS ARQUIVOS

            string texto;
            string arquivo;
            //string name = "Question1";
            Text mudar;
            int[] separacoes = new int[Texts.Count + 2];
            ...
            //Separa as partes do arquivo
            int j = 0;
            separacoes = new int[1 + Texts.Count];
            for (int i = 0; i < texto.Length && j < Texts.Count + 1; i++)
            {
                if (texto[i] == '\n')
                {
                    separacoes[j] = i;
            ...
            mudar = additionalElements1[0].GetComponent(typeof(Text)) as Text;
            mudar.text = texto.Substring(0, separacoes[0] - 1);


            //Coloca as opcoes
            for (int i = 0; i < Texts.Count; i++)
            {
                questions[i].text = texto.Substring(separacoes[i] + 1, separacoes[i + 1] - separacoes[i]);
            }


            //coloca a frase da resposta
            mudar = additionalElements3[0].GetComponent(typeof(Text)) as Text;
            mudar.text = texto.Substring(separacoes[Texts.Count], texto.Length - separacoes[Texts.Count]);
            ...
            if (!loaded)
            {
                ...
                if (j == 8) //ultima execucao sai pela condicao 1 e fica sem 9 termos
                    separacoes2[j] = i + 1;

                //carregar opcoes do modelo
                Transform model = multiobjectimporter.gameObject.transform.Find("object" + multiobjectimporter.user_id);
                GameObject particao = new GameObject();
                particao.name = "question" + multiobjectimporter.user_id;
                particao.transform.SetParent(QuestionObjects[0].transform, false);
                particao.transform.localScale = new Vector3(1, 1, 1);
                model.transform.SetParent(particao.transform);
                multiobjectimporter.user_id++;  //aumenta o id para indicar qual o proximo modelo contido no importador
                QuestionObjects[0] = model.gameObject;
                Controler_aux = true;

                float div = this.Marker.transform.localScale.x;

                if (div != 0)
                    particao.transform.localPosition = new Vector3(float.Parse(texto.Substring(separacoes2[1] + 1, separacoes2[2] - separacoes2[1] - 1)) / div, float.Parse(texto.Substring(separacoes2[2] + 1, separacoes2[3] - separacoes2[2] - 1)) / div, float.Parse(texto.Substring(separacoes2[3], separacoes2[4] - separacoes2[3])) / div);
                else
                {
                    Debug.Log("Error, Marker scale is 0");
                }

                particao.transform.Rotate(particao.transform.localRotation.x * -1, particao.transform.localRotation.y * -1, particao.transform.localRotation.z * -1);
                particao.transform.Rotate(float.Parse(texto.Substring(separacoes2[4] + 1, separacoes2[5] - separacoes2[4])), float.Parse(texto.Substring(separacoes2[5], separacoes2[6] - separacoes2[5])), float.Parse(texto.Substring(separacoes2[6], separacoes2[7] - separacoes2[6])));
                particao.transform.localScale = new Vector3(float.Parse(texto.Substring(separacoes2[7] + 1, separacoes2[8] - separacoes2[7] - 1)), float.Parse(texto.Substring(separacoes2[7] + 1, separacoes2[8] - separacoes2[7] - 1)), float.Parse(texto.Substring(separacoes2[7] + 1, separacoes2[8] - separacoes2[7] - 1)));
    ...
    public void ResetButtonMethod()//botao de reset pergunta, se usa quando a pergunta é inicializada ou quando se pressiona o botao de reset questao  
    {
    ...
    private IEnumerator ChangeQuestion(GameObject newQuestionController)
    {
        if (Active)
        {
            ...
            else if (newQuestionController.GetComponent<I_QuestionController1>() != null)//se a proxima pergunta tem também um componente tipo QuestionControlle1 (é uma pergunta do mesmo tipo do que a atual)
            {
                newQuestionController.GetComponent<I_QuestionController1>().Active = true;//ativar o proximo QuestionController (da proxima pergunta)
            }
        }
    }
